...
pos-venezuela/
  ...
  sql/               # Migraciones numeradas 01..29 (correr en orden en SQL Server)
```
...
### 4.c — Academia VIDA → LMS completo (2026-09-21, en curso)

Se está ampliando la Academia v1 (cursos planos) a un **LMS estilo Udemy/Platzi**. Estado por fase:

- **Fase 1 (HECHA) — Modelo + backend core.** Migraciones **`sql/33_academia_lms.sql`** (estructura) y **`sql/34_academia_seed_estructura.sql`** (demo). La 33 amplía `VIDA_ACADEMIA_CURSOS` (`Tipo` CAPACITACION|NORMAS|GUBERNAMENTAL, `Obligatorio`, `FechaLimite`, `Portada`, `Visibilidad` TODOS|ROLES|USUARIOS|MIXTO, `Audiencia` EMPRESARIO|CLIENTE|AMBOS, `FechaAlta`/`UsuAlta`) y crea tablas nuevas: `VIDA_ACADEMIA_MODULOS`, `VIDA_ACADEMIA_LECCIONES` (TipoLeccion VIDEO|TEXTO|PDF|QUIZ, `VideoUrl` enlace externo **y** `ArchivoUrl` subido, `Contenido` NVARCHAR(MAX), `QuizAprob`), `VIDA_ACADEMIA_QUIZ_PREGUNTAS`/`_OPCIONES`, `VIDA_ACADEMIA_CURSO_ROLES` + `VIDA_ACADEMIA_CURSO_USUARIOS` (targeting), `VIDA_ACADEMIA_LECCION_PROGRESO` (granular con FechaInicio/FechaFin/SegundosTomados/QuizPuntaje), `VIDA_ACADEMIA_COMENTARIOS`, `VIDA_ACADEMIA_DIPLOMAS` (folio único + índice único usuario+curso). La 34 le da módulos/lecciones/quiz demo al curso 1 y siembra el curso 6 "Cumplimiento SENIAT" (GUBERNAMENTAL, obligatorio, fecha límite +30d). **Ambas aplicadas en QA** (`npm run migrate` → OK). `VIDA_ACADEMIA_CURSOS`/`_PROGRESO` v1 se conservan (compat): el progreso de curso se recomputa desde las lecciones.
  - **Código:** `academia.controller.js` (reescrito: learner) + **nuevo** `academia.admin.controller.js` (autoría/analítica) + **nuevo** `academia.logic.js` (lógica pura) + `academia.routes.js` (reescrito). Sin tocar `app.js` (la ruta ya estaba registrada). Subida de video/PDF → `uploads/academia/` (multipart, sirve por `/uploads`; nota: para video pesado conviene CDN a futuro).
  - **Endpoints.** Learner (todos los roles con la pantalla): `GET /academia/cursos` (mis cursos **filtrados por targeting**, con ProgresoPct/FueraDeTiempo/DiasRestantes + resumen), `GET /academia/cursos/:id` (módulos+lecciones+mi progreso), `POST /academia/lecciones/:id/iniciar` y `/completar` (progreso granular), `POST /academia/lecciones/:id/quiz/responder` (califica, aprueba, completa), `POST /academia/cursos/:id/completar` (compat cursos planos), comentarios (`GET/POST /academia/cursos/:id/comentarios`, `DELETE /academia/comentarios/:id`), diplomas (`GET /academia/diplomas`, `GET /academia/constancia/:folio` con QR). Autoría (roles de red): CRUD curso (`/academia/admin/cursos` GET lista/GET :id detalle/POST/PUT/DELETE), `PUT .../:id/visibilidad` (roles+usuarios), `GET /academia/admin/usuarios`, módulos (`POST /academia/admin/cursos/:id/modulos`, PUT/DELETE `/academia/admin/modulos/:id`), lecciones (`POST /academia/admin/modulos/:id/lecciones`, PUT/DELETE, `POST .../:id/video` multipart), `PUT /academia/admin/lecciones/:id/quiz` (bulk), analítica (`GET /academia/admin/analitica` totales+por curso con menor tiempo/vencidos, `GET /academia/admin/analitica/curso/:id` por usuario con % avance, tiempo y fuera de tiempo). IDs con patrón atómico `insertarConId`; visibilidad/quiz en transacción.
  - **Tests:** `backend/tests/academia.test.js` (25) — visibilidad/targeting, fuera de tiempo (con fix de zona horaria en `parseFechaLocal`), % progreso, calificación de quiz, folio. **Suite total 58/58 verde.** **E2E contra el server vivo 22/23** (el "fallo" 23 es esperado: el usuario ficticio de prueba no existe en `VIDA_CUENTA_USUARIOS`, por eso no sale en la analítica por curso, que lista usuarios reales). Curso/progreso de prueba **limpiados de QA**.
- **Fase 2 (HECHA) — Panel LMS.** `frontend/src/pages/Academia.jsx` reescrita como LMS con 3 pestañas: **Mis cursos** (catálogo filtrado por targeting con badges obligatorio/fuera de tiempo + barra de progreso, **reproductor** a pantalla completa con temario lateral, media por tipo — iframe YouTube/Vimeo, `<video>`/PDF de archivo subido, texto, quiz interactivo — y **comentarios** por curso/lección), **Gestión** y **Analítica** (solo roles de red). Editor en **nuevo** `frontend/src/pages/AcademiaAdmin.jsx`: CRUD curso con todos los campos (tipo, obligatorio+fecha límite, audiencia, puntos), **visibilidad** (selector roles + checklist de usuarios), árbol módulos→lecciones con alta/edición/baja, subida de video/PDF (multipart) o pegar enlace, y **editor de quiz** (preguntas + opciones + correcta + % aprobación). El detalle learner (`GET /academia/cursos/:id`) ahora **embebe el quiz sin el flag `EsCorrecta`** (no filtra respuestas — verificado). Build de Vite OK.
- **Fase 3 (HECHA) — Analítica + constancias.** Pestaña **Analítica**: tarjetas de totales + tabla por curso (inscritos, completaron, % avance, **menor tiempo** de completación, badge **vencido**) con drill-down por usuario (avance, tiempo, fuera de tiempo). **Constancia imprimible**: `frontend/src/pages/Constancia.jsx` + ruta `/constancia/:folio` en `App.jsx` (lámina con folio, **QR de verificación** y botón imprimir/PDF), enlazada desde "Mis constancias" en Mis cursos.
- **Fase 4 (PENDIENTE) — Pulido / exponer en app cliente** (Audiencia CLIENTE/AMBOS ya modelada; falta endpoint+pantalla en `app-cliente`).
...
- **Ubicación de tienda por mapa — backend listo (frontend pendiente)** — la dueña pidió un **mapa de Google Maps en Nueva/Editar tienda** para fijar el pin (lat/lng) y que el repartidor sepa a dónde llegar. Hallazgo: `VIDA_CUENTA_PUNTOS_VENTA` **ya tiene `Latitud`/`Longitud`** y el **delivery/despacho ya las consumen** como origen (delivery.controller ~1071/2468/2503/3170, dispatch.service ~128); solo faltaba que el alta/edición del panel las guardara. Se agregó a `sucursales.controller.js` (`crearPuntoVenta`/`editarPuntoVenta`) el guardado de `Latitud`/`Longitud` con helper `latLng(v,max)` (valida rango, fuera de rango/vacío → null) y **`COALESCE` en el UPDATE** para que el formulario viejo (que no las envía) **no borre** coordenadas existentes. Verificado: crea con coords, edita sin enviarlas → las conserva, edita con nuevas → actualiza, inválidas → null. **Pendiente (bloqueado):** el **selector de mapa en `frontend/src/pages/Sucursales.jsx`** — ese archivo lo tiene la sesión paralela sin commitear, y falta la **API key de Google Maps**. El modal ya manda el resto de la dirección; solo hay que agregar el mapa + inputs lat/lng y mandarlos.
- **Suite de tests unitarios (deuda técnica #3, parcial)** — primer `.test.js` propio del proyecto, con el runner **nativo de Node** (`node --test`, cero dependencias). `npm test` corre `backend/tests/*.test.js`. **33 tests** de la lógica pura de dinero: `evaluarCupon` (17 casos: %/$ fijo, min compra, tope de descuento, tope al subtotal, vigencia, canal, usos agotados, alcance por producto, redondeo) y `mejorPromoUnitaria`/`calcularLinea` de promociones (16 casos: mejor precio por unidad, alcance TODO/PRODUCTO/CATEGORIA, clamp a 0, combos NxM vs por-unidad, combo inválido). Son deterministas y **no tocan la BD** (`getPool` es perezoso, así que importar los controllers no conecta). **33/33 verde.** Pendiente: tests de integración contra BD de prueba tras una bandera `RUN_DB_TESTS`. Doc en `backend/tests/README.md`.
- **Fix del secreto de auditoría débil en prod (deuda técnica #5)** — `audit.service.js` resolvía el secreto HMAC como `AUDIT_SECRET || JWT_SECRET || 'audit_dev_secret'` sin validar: un `AUDIT_SECRET` débil (o el literal dev, que está en el repo) hacía las firmas de `VIDA_AUDIT_LOG` falsificables. Se agregó en `app.js` el mismo gate de arranque que ya tenía `JWT_SECRET`: en producción aborta (`process.exit(1)`) si el secreto efectivo (`AUDIT_SECRET`, o `JWT_SECRET` de respaldo) falta, tiene <32 chars o es un valor de desarrollo conocido. En desarrollo `audit.service.js` avisa por consola cuando usa el default (sin bloquear). `.env.example` documenta la exigencia. **Verificado (5 casos):** AUDIT corto→exit 1, AUDIT=dev→exit 1, sin AUDIT + JWT fuerte→pasa por respaldo, exit code real 1, y dev→warn+carga. Sin migración ni cambios de negocio.
- **Fix de IDs sin lock en inventario (deuda técnica #1, parcial)** — `inventario.controller.js` generaba ids con `SELECT MAX(id)+1` y luego un `INSERT` aparte: dos requests concurrentes leían el mismo MAX → PK duplicada. Se reemplazó por el helper `insertarConId(req, {tabla, idCol, columnas, valores})` que inserta en **una sola sentencia atómica**: `INSERT … OUTPUT inserted.<id> SELECT ISNULL(MAX(<id>),0)+1, … FROM <tabla> WITH (UPDLOCK, HOLDLOCK) WHERE idBranch=@ AND idCuenta=@`. El lock de rango se retiene durante el statement, así dos inserts concurrentes se serializan. Aplicado a **categorías, productos y movimientos** de inventario. **Verificado E2E:** alta de categoría/producto/movimiento OK, y **30 altas de categoría en paralelo → 30/30 con ids únicos y contiguos, 0 colisiones** (con el código viejo varias habrían dado 500 por PK duplicada). Ver la deuda técnica #1 en §7 para lo que falta (delivery bloqueado, catálogos de baja concurrencia, y el bug de "lock falso" en academia/premios/servicios). Sin migración.
- **Vencimiento de puntos automatizado (feature #4)** — antes solo existía el disparo manual `POST /delivery/admin/puntos/expirar-inactivos`. Ahora en `premios.controller.js` se extrajo el núcleo `expirarPuntosInactivosCore(pool, idBranch, idCuenta)` (reutilizado por el endpoint manual) y se agregó `expirarPuntosInactivosJob(pool, log)` **multi-tenant** (itera `DISTINCT idBranch,idCuenta` de `VIDA_APP_CLIENTES`). En `app.js` corre por `setInterval` **1×/día** (configurable con `PUNTOS_EXPIRACION_INTERVALO_MS`, 0 lo desactiva) y en **producción** además hace una corrida ~2 min tras arrancar (el server puede reiniciarse a diario y no llegar al intervalo). Idempotente: al vencer registra un movimiento `VENCIDO` (que actualiza la última actividad) y deja el saldo en 0, así el cliente no se re-expira. Umbral por tenant en `VIDA_CONFIG_DELIVERY.MesesInactividadVence` (0 = desactivado). **E2E 8/8** en un tenant aislado (1,999) para no tocar datos reales: expira solo al inactivo, deja intacto al activo, idempotente, respeta meses=0, y el job corre sin error. Verificado además que **hoy 0 clientes reales de (1,1) vencerían**. Sin migración (reusa tablas de puntos).
- **Migrador de esquema (deuda técnica #4)** — nuevo `backend/scripts/migrate.mjs` + scripts npm `migrate` / `migrate:status` / `migrate:baseline`. Tabla de control `VIDA_SCHEMA_MIGRATIONS` (Filename PK, Version, Checksum sha256, Baseline, DurationMs, AppliedAt/By). Descubre `sql/NN_*.sql`, ordena por prefijo numérico, aplica **solo lo pendiente** partiendo por `GO`, registra cada una y se detiene en la primera que falle (sin registrarla). `status` muestra aplicada/baseline/pendiente/**drift** (checksum cambió tras aplicarse)/huérfana; `baseline [--to=N]` adopta el migrador en una BD que ya tiene migraciones (las marca aplicadas **sin ejecutar**); `up --dry-run` lista sin tocar. Usa `backend/.env` para la conexión. **Probado en QA:** status→33 pendientes, baseline→0 pendientes (01..32 marcadas), autoprueba `up` aplicó una migración temporal y fue idempotente al reintentar. **QA ya quedó con baseline.** Docs de despliegue e instalación actualizadas para usarlo (opción recomendada; SSMS queda como fallback).
- **Cupones v1 (códigos de descuento)** — archivos NUEVOS `backend/src/controllers/cupones.controller.js` + `routes/cupones.routes.js` (registrados en `app.js`) + página `frontend/src/pages/Cupones.jsx` + ruta `/cupones` en `App.jsx`. Migración `sql/32`: tablas `VIDA_CUPONES` (catálogo) + `VIDA_CUPONES_USOS` (ledger de usos, índice único cupón+pedido = idempotencia), **ALTER `VIDA_PEDIDOS` ADD `CuponCodigo`/`CuponDescuentoUSD`**, seed `BIENVENIDO10`, e **inserta la pantalla `/cupones`** (ícono Lucide `Ticket`) con acceso a red+ADMIN. Tipos `DESCUENTO_PCT`/`DESCUENTO_USD`, con `MinCompra`, `MaxDescuento` (tope), `Alcance` (TODO/CATEGORIA/PRODUCTO), `Canal` (TODO/POS/DELIVERY), `UsosMax` (global) y `UsosPorCliente`. Endpoints: CRUD panel `GET/POST/PUT/DELETE /cupones` + `GET /cupones/:id/usos`; **preview POS** `POST /cupones/validar`; **cliente** `POST /delivery/cliente/cupones/validar` (preview, ya considera límite por cliente) y `POST /delivery/cliente/cupones/aplicar` (aplica a un pedido existente: valida contra `TotalUSD`, incrementa `UsosActuales` atómico respetando `UsosMax`, actualiza el total y registra el uso, **transaccional e idempotente**; solo si el pedido no está pagado ni finalizado). Motor puro `evaluarCupon(cupon,{subtotal,canal,items})` reutilizable. **Diferencia con Promociones (sql/13):** la promo se aplica sola por producto/categoría; el cupón exige que el cliente ingrese un CÓDIGO y tiene límites de uso. **E2E 18/18 OK** (CRUD, duplicado→409, PCT>100→400, preview POS/cliente, aplicar −desc real al pedido, reaplicar→409, límite por cliente→409, editar/desactivar). **Fase 2 (hecha):** UI de cupón en el **POS** (`frontend/src/pages/POS.jsx`: campo + validación con `/cupones/validar`, descuento en el desglose/total; la venta lleva `CuponCodigo`/`CuponDescuentoUSD` y `procesarVentaOffline` en `pedidos.controller.js` los persiste, refleja el descuento en `TotalUSD` y registra el uso Canal='POS' sin cliente, best-effort e idempotente por el UUID de la venta — E2E 10/10) y en el **checkout de la app cliente** (`app-cliente/app/(tabs)/carrito.jsx`: campo de cupón que valida con `/delivery/cliente/cupones/validar`, se recalcula al togglear puntos, y tras crear el pedido llama `/delivery/cliente/cupones/aplicar`). **Único pendiente:** integrar el cupón *dentro* de `crearPedidoApp` en un solo paso (hoy es crear-pedido→aplicar en 2 llamadas, para no chocar con la sesión paralela que aún tiene `delivery.controller.js` sin commitear).
- **Simulación "un día en la tienda" + checklist de despliegue** — flujo E2E operativo de punta a punta (`e2e-dia.mjs`, temporal, ya borrado): apertura de caja $50 → empresario crea promoción 20% → 3 ventas POS por `/pedidos/sync` (2 efectivo + 1 tarjeta con promo) → reintento idempotente (3/3 duplicado, stock no se re-descuenta) → pedido por app del cliente + entrega del repartidor (cliente gana +50 pts) → **resumen y cierre de caja cuadrando ($9.36 / 3 tx)**. Resultado **16/16 OK**. Hallazgo: **NO era bug de backend** — el `$0` que apareció primero fue un error del arnés de prueba (leía `TotalVentas` al nivel equivocado; el endpoint lo anida en `res.data.ventas`). `calcularTotales` y la ventana de fechas (`FechaApertura`/`FechaAlta` ambas `GETDATE()`, sin desajuste UTC) están correctos; **sin cambios de código**. Nuevo doc **`DESPLIEGUE-VIDA.md`** (checklist paso a paso: BD → backend IIS → panel Vite → apps EAS → smoke test → rollback). (Nota histórica: cuando se corrió este flujo aún **no existía el módulo de cupones**; ya se construyó después — ver la entrada "Cupones v1" arriba.)
- **Auditoría E2E (69 casos, 3 actores)** — simulación de punta a punta de todos los flujos (panel + cliente + repartidor) contra el backend vivo, con aserciones y verificación en BD. Resultado 69/69 tras un ajuste: se restringió `/reportes/red` a roles de red (`preRed` ya no incluye `'ADMIN'`; no había fuga porque `buildGeoFilter` ya scopeaba, es defensa/consistencia). Detalle completo en **`AUDITORIA-VIDA.md`**.
- **CRUD de catálogos desde el panel** (corporativo) — 3 tabs nuevos en `Operaciones.jsx` con un componente genérico `CrudCatalogo` (lista + modal alta/edición + baja lógica): **Catálogo premios**, **Cursos (Academia)**, **Operadoras**. Backend: se agregaron los CRUD admin (roles de RED) — premios `GET/POST/PUT/DELETE /delivery/admin/premios`; cursos `.../academia/admin/cursos`; operadoras `.../delivery/admin/operadoras`. DELETE = baja lógica (Status/Activo). **Sin migración** (reusa tablas + la pantalla `/operaciones`). Con esto corporativo ya no depende de SQL para cargar premios/cursos/operadoras.
- **Panel admin unificado "Operaciones"** (panel) — nueva página `frontend/src/pages/Operaciones.jsx` + ruta `/operaciones` en `App.jsx`. Migración `sql/29` inserta la pantalla `/operaciones` (ícono `ClipboardList`) y da acceso a roles de RED (SUPER_ADMIN/ADMIN_PAIS/ADMIN_ESTADO). 3 tabs: **Recargas** (lista PROCESANDO + Completar/Rechazar), **Canjes de premios** (lista PENDIENTE + Entregar/Cancelar), **Vencimiento de puntos** (botón que ejecuta el expirado). Se agregaron los GET admin que faltaban: `GET /delivery/admin/servicios?status=` y `GET /delivery/admin/premios/canjes?status=` (los PATCH de acción y el expirar ya existían). **No requiere backend nuevo salvo esos 2 listados.** Pendiente: CRUD de catálogos (premios, cursos, operadoras) desde el panel.
...
`16_ciudades.sql`, `17_razonsocial_lada.sql`, `18_producto_plus.sql`, `19_tienda_finanzas.sql`, `20_puntos_fidelizacion.sql`, `21_canje_puntos.sql`, `22_tienda_metas.sql`, `23_hidratacion.sql`, `24_hidratacion_bonus_idempotente.sql`, `25_club_membresia.sql`, `26_servicios_recargas.sql`, `27_academia_vida.sql`, `28_premios_vencimiento.sql`, `29_panel_operaciones.sql`, `32_cupones.sql`. Checklist completo en **`DESPLIEGUE-VIDA.md`**.
> Nota: `30_liberar_pedido.sql` y `31_renombrar_tiendas.sql` son de la sesión paralela; correr en el número que les toque.
...
1. **`MAX(id)+1` sin lock** — **PARCIAL.** `inventario.controller.js` ya quedó **atómico**: se reemplazó `nextId`+`INSERT` (dos statements sin lock efectivo) por un helper `insertarConId` que hace `INSERT … OUTPUT … SELECT ISNULL(MAX)+1 … WITH (UPDLOCK, HOLDLOCK)` en **una sola sentencia** (categorías, productos y movimientos). **Probado con 30 inserts concurrentes: 0 colisiones**, ids únicos y contiguos (antes varias habrían dado PK duplicada / 500). **Pendiente aplicar el mismo patrón** a: `acreditarPuntosCliente` y demás inserts de `delivery.controller.js` (bloqueado hoy por la sesión paralela sin commitear); los catálogos de baja concurrencia (`paises/estados/ciudades/corporativo/promociones/proveedores/sucursales`, `usuarios.controller.js:281`); y **ojo**: `academia/premios/servicios` ponen `WITH (UPDLOCK, HOLDLOCK)` sobre `pool.request()` **sin transacción** → el lock NO se retiene (falsa sensación de seguridad); usar `insertarConId` (patrón de referencia en `inventario.controller.js`) o `nextIdTx` dentro de una transacción.
...
3. ~~**Cero tests.**~~ **PARCIAL.** Ya hay suite con el runner nativo `node --test` (sin dependencias): `npm test` corre `backend/tests/*.test.js`. **33 tests** de la lógica pura de dinero — `evaluarCupon` (cupones) y `mejorPromoUnitaria`/`calcularLinea` (promos + combos NxM), deterministas y sin BD. **Pendiente:** tests de integración contra BD de prueba (flujos que hoy se validan con scripts E2E manuales), idealmente detrás de una bandera `RUN_DB_TESTS`. Ver `backend/tests/README.md`.
4. ~~**Sin migrador.**~~ **RESUELTO.** Ya hay migrador con tabla de control `VIDA_SCHEMA_MIGRATIONS` (`backend/scripts/migrate.mjs`): `npm run migrate:status` / `migrate` / `migrate:baseline`. Aplica solo lo pendiente en orden, respeta los `GO`, registra checksum+duración y detecta drift. QA ya quedó con baseline 01..32. Ver la entrada en §4.b y el uso en `DESPLIEGUE-VIDA.md`/`INSTALACION.md`.
5. ~~**`audit.service.js:9`** cae a `'audit_dev_secret'`~~ **RESUELTO.** `app.js` ahora aborta el arranque en producción si el secreto de auditoría efectivo (`AUDIT_SECRET`, o `JWT_SECRET` de respaldo) falta, es <32 chars o es un valor de desarrollo conocido — mismo gate que `JWT_SECRET`. En dev se avisa por consola pero no bloquea. Verificado (5 casos: débil/dev→exit 1, respaldo a JWT fuerte→pasa, dev→warn).
...
4. ~~**Vencimiento de puntos: automatizar**~~ **HECHO** — job diario en `app.js` (`expirarPuntosInactivosJob`, multi-tenant) + el endpoint manual sigue existiendo. Configurable con `PUNTOS_EXPIRACION_INTERVALO_MS`. Pendiente opcional: vencimiento FIFO por lote en vez de por inactividad.
